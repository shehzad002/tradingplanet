...
const FarmCards: React.FC = () => {
  ...
  const tpIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'TP',
  )

  const tpPrice =
    tpIndex >= 0 && stakedValue[tpIndex]
      ? stakedValue[tpIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const TP_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [tp, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'TP' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
